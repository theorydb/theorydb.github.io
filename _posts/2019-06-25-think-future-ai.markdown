...
## 개요
> 통계학과 컴퓨터공학이 보이지 않는곳에서 작은 다툼(?)을 하고 있습니다. 현 시점만 놓고 봤을 때 이 작은 다툼은 양측이 충분한 일리를 가진 의미있는 싸움이라고 생각하는데요. 이 부분에 초점을 맞추어 데이터 과학의 미래가 어떻게 변화할지 그동안의 경험을 통한 감(感)과 공상과학(?)을 가미하여 다소 건방져 보일지 몰라도 소신있게 예측해 보려합니다.

- 목차
	- [딥러닝의 최대 약점(?)](#딥러닝의-최대-약점)
	- [통계학 : Data Science? AI? 모두 예전에 우리가 했던 일이다.](#통계학-Data-Science-AI-모두-예전에-우리가-했던-일이다.) 
	- [컴퓨터공학 : 수학, 통계학이던 Data던 전부 컴퓨터 안에 집어넣으면 그만이다.](#컴퓨터공학-수학-통계학이던-Data던-전부-컴퓨터-안에-집어넣으면-그만이다)
...
## 딥러닝의 최대 약점(?)
---
네 까짓게 감히 알파고 이후로 누구나 떠받드는 위대한 기술을 폄하해 보겠다고? 감히 나 따위가 어떻게 폄하하겠는가. 하지만 `모두 다 Yes라고 할 때 No`라고 대답할 수 있게끔 반대 시각에서 생각해보는 습관은 언제나 의미있다고 생각한다. 요즘 우스개 소리로 기업에서는 상부에 올린 결재가 반려될 경우 "AI, 딥러닝, 빅데이터"라는 마법의 단어를 사용하면 결재를 득할 수 있다고 풍자할 정도로 A.I는 분명 트렌드의 중심에 있다.   

인류가 그동안 쌓아온 어떤 지식이나 연구결과에 비해서 딥러닝이 유독 강점을 보이는 분야는 "동영상, 이미지, 음성인식, NLP" 등의 분야다.   

그렇다면 이 분야의 공통점은 무엇일까? 대부분 비정형데이터이면서 동시에 `2차 가공을 거친 데이터`들이 집결된 분야이다. 이 세상 그대로의 데이터를 `컴퓨터가 이해할 수 있도록 전달할 방법이 없기에` 아날로그를 디지털화 하는 등 일종의 "변환" 작업을 거친다.   

그 과정에서 Min ~ Max의 경계선을 갖는 일종의 범주 형태로 변환이 되고 그 과정에서 Computing Vision, 신호처리, 푸리에 변환은 물론이고 품질의 손실을 최소화 하기 위한 연구 등 `인류가 장기간 축적해 온 지식들이 피처 및 모델에 함축되어있다.` 

그렇게 이 분야의 데이터는 유한한 범위를 가짐과 동시에 컴퓨팅 파워만 충분하다면 연산이 가능해졌고 `피처 선정 및 추출, 모델의 선택에 있어 경우의 수를 크게 좁혀준 셈이다.`  
...
## 컴퓨터공학 : 수학, 통계학이던 Data던 전부 컴퓨터 안에 집어넣으면 그만이다.  
---
